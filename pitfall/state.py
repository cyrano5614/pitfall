...
from anytree import AbstractStyle, ContStyle, NodeMixin, RenderTree, findall_by_attr
...
class PulumiResource(NodeMixin):
    ...
    def __repr__(self):
        s = (
            "PulumiResource(urn=%r, rtype=%r, rid=%r, provider=%r, inputs=%r, outputs=%r, dependencies=%r, parent=%r)"
            % (self.urn, self.type, self.id, self.provider, self.inputs, self.outputs, self.dependencies, self.parent)
        )
        return s
    ...
    def __post_init__(self) -> None:
        ...
        self.new = {
            ...
            "checkpoint": {
                "stack": self.stack,
                "latest": {
                    "manifest": {"time": timestamp, "magic": self.magic_cookie, "version": self.pulumi_version},
                    "secrets_provider": {"type": "passphrase", "state": {"salt": self.encryptionsalt}},
                },
            },
        }
