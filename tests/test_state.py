...
from pitfall.state import PulumiResource, PulumiResources, PulumiState
...
class TestPulumiResources(unittest.TestCase):
    def setUp(self):
        ...
        self.second = PulumiResource(
            urn="test-vpc", rtype="aws:ec2/vpc:Vpc", rid="vpc-001", provider=provider, parent=self.first
        )
        self.third = PulumiResource(
            urn="test-subnet-1", rtype="aws:ec2/subnet:Subnet", rid="subnet-001", provider=provider, parent=self.second
        )
        self.fourth = PulumiResource(
            urn="test-bucket", rtype="aws:s3/bucket:Bucket", rid="test-s3-bucket", provider=provider, parent=self.first
        )
        self.fifth = PulumiResource(
            urn="test-subnet-2", rtype="aws:ec2/subnet:Subnet", rid="subnet-002", provider=provider, parent=self.second
        )
...
class TestPulumiState(unittest.TestCase):
    def setUp(self):
        ...
        self.pulumi_state = PulumiState(stack="unit-test", encryptionsalt=encryptionsalt)
    ...
    def test_pulumi_version(self):
        ...
        completed_process = subprocess.CompletedProcess(
            args=["pulumi", "version"], returncode=0, stdout=stdout, stderr=None
        )
    ...
    def test_pulumi_version_raises_exception(self):
        ...
        completed_process = subprocess.CompletedProcess(
            args=["pulumi", "version"], returncode=255, stdout=b"", stderr=stderr
        )
